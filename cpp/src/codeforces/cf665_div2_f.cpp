...
private:
	...
	Integer _query(ll start, ll end, ll searchStart, ll searchEnd, ll node, ll depth) const
	{
		...
		return op.Execute(
			_query(start, end, searchStart, (searchStart + searchEnd) / 2, left, depth + 1),
			_query(start, end, (searchStart + searchEnd) / 2, searchEnd, right, depth + 1));
	}

public:
	SegmentTree(ull maxSize) : maxSize_(1), reverse_bit(0)
	{
	...
	Integer query(ll start, ll end) const
	{
		...
		return _query(start, end, 0, maxSize_, 0, 0);
	}

	void update_reverse_bit(ll query_bit)
	{
		reverse_bit ^= query_bit;
	}
...
const ll SumOp::default_value = 0;

using RMQ = SegmentTree<ll, SumOp>;
...
int solve()
{
	...
	RMQ rmq(1LL << n);
	RMQ idx_holder(1LL << n);
	ll a;
	REPD(i, 0, 1LL << n)
	{
		cin >> a;
		rmq.update(i, a);
		idx_holder.update(i, i);
	}
	...
	REPD(i, 0, q)
	{
		...
		if (t == 1)
		{
			cin >> x >> k;
			--x;
			auto idx = idx_holder.query(x, x + 1);
			rmq.update(idx, k);
		}
		else if (t == 2)
		{
			...
			rmq.update_reverse_bit(query_bit);
			idx_holder.update_reverse_bit(query_bit);
		}
		else if (t == 3)
		{
			cin >> k;
