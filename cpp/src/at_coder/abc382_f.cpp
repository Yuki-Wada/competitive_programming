...
using quadruple = tuple<ll, ll, ll, ll>;
ll H, W, N;

int solve()
{
	cin >> H >> W >> N;
	vector<quadruple> quadruples(N);
	REPD(i, 0, N)
	{
		ll r, c, l;
		cin >> r >> c >> l;
		--c;
		quadruples[i] = {H - r, i, c, l};
	}
	sort(quadruples.begin(), quadruples.end());

	RMQLP rmq(W);
	vector<llpair> results;
	REPD(i, 0, N)
	{
		auto [r, idx, c, l] = quadruples[i];
		auto h = rmq.query(c, c + l);
		rmq.update(c, c + l, h + 1LL);
		results.emplace_back(idx, H - h);
	}
