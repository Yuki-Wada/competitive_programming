...
{
	Integer result = Polynomial<rll>(1);
	while (exponential >= 1)
	{
		if (exponential & 1)
		{
...
ll solve()
{
	...
	while (true)
	{
		...
		if (n >= pow10)
		{
			nums.emplace_back((pow10 - 1LL) - prev);
			prev = pow10 - 1LL;
		}
		else
		{
			nums.emplace_back(n - prev);
			break;
		}
	}

	auto excls = get_exclamations(n);

	Polynomial<rll> base_poly(1LL);
	{
		rll pow10_rll = 1LL;
		REPI(num, nums)
		{
			pow10_rll *= rll(10LL);
			Polynomial<rll> p({1, pow10_rll});
			base_poly = base_poly * get_power(p, num - 1LL);
			base_poly.zero_trim();
		}
	}
	...
	rll res = 0;
	{
		...
		REPD(i, 0, nums.size())
		{
			Polynomial<rll> residual_poly(1LL);
			rll pow10_rll_internal = 1LL;
			REPD(j, 0, nums.size())
			{
				pow10_rll_internal *= 10LL;
				if (i == j)
				{
					continue;
				}
				residual_poly = residual_poly * Polynomial<rll>({1, pow10_rll_internal});
			}

			residual_poly = base_poly * residual_poly;
			residual_poly.zero_trim();

			ll num = nums[i];
			rll partial_res = 0LL;

			REPD(i, 0, residual_poly.coef_num())
			{
				partial_res = partial_res + residual_poly.coefs[i] * excls[i] * excls[n - i - 1LL];
			}

			rll start = pow10_rll;
			rll end = pow10_rll + num - rll(1LL);
			partial_res = partial_res * (end - start + 1LL) * (start + end) / rll(2LL);
