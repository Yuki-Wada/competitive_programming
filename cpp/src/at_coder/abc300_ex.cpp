...
pair<Polynomial<rll>, Polynomial<rll>> calc(const Polynomial<rll> &px, const Polynomial<rll> &qx, ll n)
{
    auto qmx = Polynomial<rll>::zero_polynomial(K + 1LL);
    REPD(i, 0, K + 1LL)
    qmx[i] = qx[i] * (i % 2LL == 0LL ? 1LL : -1LL);

    auto qxqmx = qx * qmx;
    auto tx = Polynomial<rll>::zero_polynomial(K + 1LL);
    REPD(i, 0, K + 1LL)
    tx[i] = qxqmx[i * 2LL];

    auto pxqmx = px * qmx;
    auto sx = Polynomial<rll>::zero_polynomial(K);
    REPD(i, 0, K)
    sx[i] = pxqmx[i * 2LL] + (n % 2LL == 1LL ? pxqmx[i * 2LL + 1LL] : rll(0LL));
...
{
    cin >> K >> N;
    auto px = Polynomial<rll>::zero_polynomial(K);
    auto qx = Polynomial<rll>::zero_polynomial(K + 1LL);
    qx[0] = 1;
    REPD(i, 0, K)
    px[i] = 1 - i, qx[i + 1LL] = -1;
    ...
    while (N > 0LL)
    {
        auto res = calc(px, qx, N);
        px = res.first, qx = res.second;
        // cout << px.coef_num() << " " << qx.coef_num() << " " << endl;
        N /= 2LL;
    }
