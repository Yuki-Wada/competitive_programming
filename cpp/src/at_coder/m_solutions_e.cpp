...
static constexpr ll MOD = 1000003LL;
...
ll Q;
auto excls = get_exclamations(MOD - 1LL);

rll calc(rll rdx, ll n)
{
	if (rdx.n() + n - 1LL >= MOD)
	{
		return 0LL;
	}
	rll res = excls[rdx.n() + n - 1LL] / excls[rdx.n() - 1LL];
	return res;
}
...
int solve()
{
	...
	REPD(i, 0, Q)
	{
		...
		{
			rll rxd = rll(x) / rll(d);
			ll res = (calc(rxd, n) * get_power(rll(d), n)).n();
			cout << res << endl;
		}
		else
		{
			ll res = (get_power(rll(x), n)).n();
			cout << res << endl;
		}
	}
