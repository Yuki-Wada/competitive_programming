...
int solve()
{
	...
	ll array_size = get_highest_one_bit(n + m - 1LL);
	if (array_size < n + m - 1LL)
	{
		array_size *= 2LL;
	}
	vector<ll> as(array_size, 0), bs(array_size, 0);
	REPD(i, 0, n)
	cin >> as[i];
	REPD(i, 0, m)
	cin >> bs[i];

	auto trans_as = fmt(as);
	auto trans_bs = fmt(bs);

	REPD(i, 0, array_size)
	{
		trans_as[i] *= trans_bs[i];
		trans_as[i] %= default_number_theoretic_transform_mod;
	}
