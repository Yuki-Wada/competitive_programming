...
public:
	ll connectedCount;
	UnionFindTree(unsigned elemCount) : connectedCount(elemCount), elemCount_(elemCount),
...
int solve()
{
	...
	auto ufs = make_multiple_vector(UnionFindTree(N), 10);
	ll a, b, c;
	REPD(i, 0, N - 1LL)
	{
		cin >> a >> b >> c;
