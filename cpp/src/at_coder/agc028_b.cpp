...
int main()
{
	...
	vector<ll> reverse_accums(N + 1);
	rll reverse_accum = 0;
	for (ll i = 0; i < N; ++i) {
		reverse_accum += rll(1) / (i + 1);
		reverse_accums[i + 1] = (reverse_accum * factorial).get_n();
	}

	rll res = 0, val;
	for (ll i = 0; i < N; ++i) {
		val = as[i];
		val *= rll(reverse_accums[N - i]) + rll(reverse_accums[i + 1]) - factorial;
		res += val;
	}
