...
ll solve()
{
	ll n;
	cin >> n;
	auto a = make_multiple_vector<2, ll>(n, 100001, 0);
	REPD(i, 0, n)
	{
		ll k;
		cin >> k;
	...
	ll1d length(n);
	auto dice_rolls = make_multiple_vector<2, ll>(n, 0, 0);
	REPD(i, 0, n)
	{
		REPD(j, 0, 100001)
		{
	...
	REPD(i, 0, n)
	{
		...
		{
			long double val = 0.0;
			REPD(k, 0, dice_rolls[j].size())
			{
				auto idx = dice_rolls[j][k];
				val += a[i][idx] * a[j][idx];
			}
			val /= length[i] * length[j];
