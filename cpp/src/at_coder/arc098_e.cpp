...
inline ll get_result(ll a, ll b, ll c, ll d) {
	return max(max(max(a, b), c), d) - min(min(min(a, b), c), d);
}
...
int solve() {
	...
	REPD(i, 0, N) {
		vector<ull> sizes;
		ll group_count = 0, size = 0;
		priority_queue<pair<ll, ll>> pq;
		
		REPD(j, 0, N) {
			if (As[j] >= As[i]) {
				pq.emplace(-As[j], group_count);
				++size;
			}
			else if (size > 0) {
		...
		while (!pq.empty()) {
			...
			if (operation_count >= Q) break;
		}
		if (operation_count < Q) continue;

		res = min(res, curr - As[i]);
	}
