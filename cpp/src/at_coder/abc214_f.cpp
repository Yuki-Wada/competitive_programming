...
int solve()
{
	...
	auto next_i = make_multiple_vector(-1, s.size(), 26);
	REPD(c, 0, 26)
	{
		ll p = -1, ch;
		REPD(i, 0, s.size())
		{
			ch = ll(s[s.size() - i - 1LL] - 'a');
			if (ch == c)
				p = s.size() - i;
			next_i[s.size() - i - 1LL][c] = p;
		}
	}

	auto dps = make_multiple_vector(0LL, s.size() + 1LL);
	dps[0] = 1;
