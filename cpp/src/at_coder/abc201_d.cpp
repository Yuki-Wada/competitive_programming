...
int solve()
{
	...
	{
		cin >> masses[h - i - 1LL];
		reverse(masses[h - i - 1LL].begin(), masses[h - i - 1LL].end());
	}

	auto dps = make_multiple_vector(-1000000000LL, h, w);
	dps[0][0] = 0;
	...
	REPD(i, 0, h)
	{
		REPD(j, 0, w)
		{
			if (i == 0 && j == 0)
				continue;
			if (i >= 1)
				dps[i][j] = max(dps[i][j], -dps[i - 1][j] + (masses[i - 1][j] == '+' ? 1LL : -1LL));
			if (j >= 1)
				dps[i][j] = max(dps[i][j], -dps[i][j - 1] + (masses[i][j - 1] == '+' ? 1LL : -1LL));
		}
	}
	...
	{
		bool is_takahashi = dps[h - 1][w - 1] > 0;
		cout << (is_takahashi ? "Takahashi" : "Aoki") << endl;
	}
