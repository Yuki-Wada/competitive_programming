...
const pair<ll, ll> AddOp::default_value = make_pair(0LL, 0LL);
using RMQ = SegmentTree<pair<ll, ll>, AddOp>;
...
int solve()
{
	...
	REPD(i, 0, N)
	{
		...
		auto val = rmq.query(coord, coord + 1LL);
		rmq.update(coord, make_pair(val.first + compression_table[coord], val.second + 1LL));
	}
	...
	{
		auto coord = compressed_coords[i];
		auto val = rmq.query(coord, N + 1LL);
		res +=  val.first - compression_table[coord] * val.second;

		val = rmq.query(coord, coord + 1LL);
		rmq.update(coord, make_pair(val.first - compression_table[coord], val.second - 1LL));
	}
