...
int solve()
{
	ll N;
	cin >> N;
	auto As = make_multiple_vector(0, N);
	REPD(i, 0, N)
	{
		cin >> As[i];
	}

	auto lengths = make_multiple_vector(0LL, 11LL);
	REPD(i, 0, N)
	{
		auto a = As[i];
		auto a_length = to_string(a).size();
		lengths[a_length]++;
	}
	auto powers10 = make_multiple_vector(rll(1), 11LL);
	REPD(i, 0, 10)
	{
		powers10[i + 1] = powers10[i] * 10;
	}
	...
	REPD(i, 0, N)
	{
		...
		REPD(j, 1, 11)
		{
			partial += powers10[j] * lengths[j] * a;
		}
		res += partial;
