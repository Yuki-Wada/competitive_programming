...
int solve()
{
	...
	auto component_numbers = get_scc(g);
	vector<vector<ll>> components(*max_element(component_numbers.begin(), component_numbers.end()) + 1LL);
	for (ull i = 0; i < component_numbers.size(); ++i)
	{
		components[component_numbers[i]].emplace_back(i);
	}
	...
	for (ull i = 0; i < components.size(); ++i)
	{
		...
		cout << " ";
		for (ull j = 0; j < components[i].size(); ++j)
		{
			cout << components[i][j];
			if (j + 1ULL < components[i].size())
				cout << " ";
		}
		cout << endl;
	}
