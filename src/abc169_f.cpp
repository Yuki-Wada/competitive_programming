...
ll calc_dps(ll i, ll s, const vector<ll>& as, vector<vector<ll>>& dps) {
	if (s < 0) return 0;
	if (dps[i][s] != -1) return dps[i][s];

	rll res = 0;
	res += calc_dps(i - 1, s - as[i - 1], as, dps);
	res += calc_dps(i - 1, s, as, dps) * 2;
...
int solve() {
	...
	vector<vector<ll>> dps(n + 1, vector<ll>(s + 1, -1));
	dps[0][0] = 1LL;
	for (ll i = 1; i <= n; ++i) {
		dps[i][0] = (rll(dps[i - 1][0]) * 2).get_n();
	}
	for (ll i = 1; i <= s; ++i) {
		dps[0][i] = 0;
	}
	ll res = calc_dps(n, s, as, dps);
