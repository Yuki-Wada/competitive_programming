...
int solve() {
	...
	ll left_edge_count = 0, right_edge_count = 0;
	vector<pair<ll, ll>> brackets;
	brackets.reserve(n);
	for (ll i = 0; i < n; ++i) {
		ll left = 0, right = 0;
		...
		if (left == 0) {
			left_edge_count += right;
		}
		else if (right == 0) {
			right_edge_count += left;
		}
		else {
	...
	ll left_sum = right_edge_count, right_sum = left_edge_count;
	for (ll i = 0; i < brackets.size(); ++i) {
		left_sum += brackets[i].first;
		right_sum += brackets[i].second;
	}
	...
	ll left_cat_index = -1;
	ll right_cat_index = -1;
	for (ll i = 0; i < brackets.size(); ++i) {
		if (left_edge_count >= brackets[i].first) {
			if (left_cat_index == -1) {
				left_cat_index = i;
			}
			else {
		...
		if (right_edge_count >= brackets[i].second) {
			if (right_cat_index == -1) {
				right_cat_index = i;
			}
			else {
				right_cat_index = -2;
	...
	if (left_cat_index == -1 || right_cat_index == -1) {
		cout << "No" << endl;
		return 0;
	}

	if (left_cat_index != -2 && right_cat_index != -2 && left_cat_index == right_cat_index) {
		cout << "No" << endl;
		return 0;
	}
